...
//        offlineByUserId(id); // 加上这段代码, 禁用用户后, 会将当前在线的用户立即踢出.
	...
	public User selectOneByOpenId(String openId) {
		...
		return userMapper.selectOneByOpenId(openId);
	}

	@Override
	public int insertWeChatUser(User user) {
		String salt = generateSalt();
        String encryptPassword = new Md5Hash(user.getPassword(), salt).toString();
        user.setPassword(encryptPassword);
        user.setSalt(salt);
		// TODO Auto-generated method stub
		return userMapper.insertWeChatUser(user);
	}

	@Override
	public int updateWeChatUser(User user) {
		// TODO Auto-generated method stub
		return userMapper.updateWeChatUser(user);
	}
